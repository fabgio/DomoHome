...
public class ReadThread extends Thread {
	...
	public void run() {
		...
		ci = 0;
		try {
			do if (socket != null && !socket.isInputShutdown()) {
				ci = input.read();
				if (ci == -1) {
					num = 0;
					indice = 0;
		...
		if (esito) {
			responseString = String.valueOf(risposta, 0, indice + 1);
			if(tipoSocket == 0){
		...
		System.out.println("Thread ricezione stringa terminato");
		socket = null;
		input = null;
		risposta = null;
	}
